...
public class JdbcSteamGamesDao implements SteamGamesDao {
    ...
    public List<SteamGame> getListOfGamesByName(String name) {
        String sql = "SELECT * " + "FROM steam_game " + "WHERE lower(regexp_replace(name, '[^a-zA-Z0-9]+', '', 'g')) LIKE ALL (" + "    SELECT '%' || lower(regexp_replace(unnest(string_to_array(?, ' ')), '[^a-zA-Z0-9]+', '', 'g')) || '%' " + ");";
    ...
    @Override
    public SteamGame getGameByGameID(int game_id) {
        String sql = "SELECT * " + "FROM steam_game " + "WHERE game_id = ?";
        SteamGame steamGame = new SteamGame();
        try {
            SqlRowSet result = jdbcTemplate.queryForRowSet(sql, game_id);
            if (result.next()) {
                steamGame = mapRowToSteamGame(result);
            }
        } catch (SQLException e) {
            throw new CustomException("Error retrieving steam game by game ID");
        }
        return steamGame;
    }
...
//         release year
    ...
    public void batchCreateSteamGames(final Collection<SteamGame> steamGames) {
        ...
        try {
            jdbcTemplate.batchUpdate("INSERT INTO steam_game (game_id, name, release_date, required_age, " + "price, about, image_url, is_for_windows, is_for_mac, is_for_linux, developers, publishers, categories, genres, tags) " + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", steamGames, 100, (PreparedStatement ps, SteamGame steamGame) -> {
    ...
    private void readLinesFromListAndPopulateSet(List<String> listOfStringsFromBufferedReader, Set<SteamGame> setOfSteamGamesToPopulate, HashMap<String, Integer> columnMap) {
        ...
        for (int i = 0; i < listOfStringsFromBufferedReader.size(); i++) {
            ...
            String[] developers = Arrays.stream(splitLine[columnMap.get("Developers")].split(",")).map(String::trim).map(s -> s.replaceAll("^\"|\"$", "")).toArray(String[]::new);

            String[] publishers = Arrays.stream(splitLine[columnMap.get("Publishers")].split(",")).map(String::trim).map(s -> s.replaceAll("^\"|\"$", "")).toArray(String[]::new);

            String[] categories = Arrays.stream(splitLine[columnMap.get("Categories")].split(",")).map(String::trim).map(s -> s.replaceAll("^\"|\"$", "")).toArray(String[]::new);

            String[] genres = Arrays.stream(splitLine[columnMap.get("Genres")].split(",")).map(String::trim).map(s -> s.replaceAll("^\"|\"$", "")).toArray(String[]::new);

            String[] tags = Arrays.stream(splitLine[columnMap.get("Tags")].split(",")).map(String::trim).map(s -> s.replaceAll("^\"|\"$", "")).toArray(String[]::new);
